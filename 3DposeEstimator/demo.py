...
if __name__ == "__main__":
  ...
  # 归一化处理 Normalization of the picture (x and y axis has the same scale)
  Xx, Xy = pose2D.normalization(Xx, Xy)
  save("data/demo1.txt", [Xx, Xy, Xw])

  # 删除缺失姿态点较多的姿态模型 Delete all skeletal models which have a lot of missing parts.
  Xx, Xy, Xw = pose2D.prune(Xx, Xy, Xw, (0, 1, 2, 3, 4, 5, 6, 7), 0.3, dtype)
  save("data/demo2.txt", [Xx, Xy, Xw])
  
  # 加权线性处理 Preliminary filtering: weighted linear interpolation of missing points.
  ...
  save("data/demo5.txt", [Yx, Yy, Yz])#h5
